...
These changes are necessary because what we're technically documenting is not a function as such, but an arbitrary R object (the `NULL` at the bottom of the block). The `@usage`, `@param` tags et al only apply to functions, and if you use them here, `R CMD check` will generate a warning. This can be important if you want to publish your package on CRAN.
