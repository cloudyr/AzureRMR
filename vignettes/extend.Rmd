...
public=list(
                             ...
                             resource_types="sco", ip=NULL, protocol=NULL, key=NULL)
    {
        parms <- list(keyToSign=key,
                      ...
                      signedStart=start)
        self$
            do_operation("listAccountSas", body=parms, encode="json", http_verb="POST")$
            accountSasToken
    },
...
Once you've created your new class(es), you should add convenience functions to `az_resource_group` (and optionally `az_subscription` as well, if your service has subscription-level API calls) to create, get and delete resources. Note that if you are writing a package that extends AzureRMR, these methods _must_ be defined in the package's `.onLoad` function. This is because the methods must be added at runtime, when the user loads your package, rather than at compile time, when it is built or installed.
...
This is done in the AzureStor package by defining a number of (S3) classes, representing blob and file storage endpoints, and individual containers and shares within an endpoint. It further defines (S3) methods for these classes to perform common operations like listing files, uploading and downloading files, and so on.
